return require('lib/tap')(function (test)
  ...
  test("fs.get_system_error", function(print, p, expect, uv)
    local req
    req = uv.fs_open('some-file-that-doesnt-exist', 'r', tonumber('644', 8), function(err, fd)
      p("in cb", req)
      p(uv.fs_get_system_error(req))
    end)
    p("before cb", req)
    p(uv.fs_get_system_error(req))
  end, "1.38.0")
